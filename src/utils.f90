...
module utils
  ...
  contains
    ...
    pure function u_to_w(u,g) result(w)
      ...
      w(2) = u(2)/u(1)
      w(3) = u(3)/u(1)
      w(4) = (g-1.0d0)*(u(4)-0.5d0*(u(2)**2 + u(3)**2)/u(1))
    end function u_to_w
      ...
      do j=1,nj
        do i=1,ni
          if ((i.eq.1).and.(j.eq.1)) then
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i+1,j+1,k))
            end do
            ...
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i,j+1,k),u(i-1,j,k),u(i-1,j+1,k))
            end do
            ...
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i,j-1,k),u(i+1,j+1,k),u(i+1,j-1,k))
            end do
          else if (i.eq.ni) then
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i,j+1,k),u(i-1,j,k),u(i,j-1,k),u(i-1,j+1,k),u(i-1,j-1,k))
            end do
          else if (j.eq.1) then
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i-1,j,k),u(i-1,j+1,k),u(i+1,j+1,k))
            end do
          else if (j.eq.nj) then
            do k=1,4
              um(i,j,k) = max(u(i,j,k),u(i+1,j,k),u(i-1,j,k),u(i,j-1,k),u(i+1,j-1,k),u(i-1,j-1,k))
            end do
          else
            do k=1,4
    ...
    pure function compute_elem_min(u,ni,nj) result(um)
      ...
      do j=1,nj
        do i=1,ni
          if ((i.eq.1).and.(j.eq.1)) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i+1,j+1,k))
            end do
          else if ((i.eq.1).and.(j.eq.nj)) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i+1,j,k),u(i,j-1,k),u(i+1,j-1,k))
            ...
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i,j+1,k),u(i-1,j,k),u(i-1,j+1,k))
            end do
          else if ((i.eq.ni).and.(j.eq.nj)) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i-1,j,k),u(i,j-1,k),u(i-1,j-1,k))
            ...
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i,j-1,k),u(i+1,j+1,k),u(i+1,j-1,k))
            end do
          else if (i.eq.ni) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i,j+1,k),u(i-1,j,k),u(i,j-1,k),u(i-1,j+1,k),u(i-1,j-1,k))
            end do
          else if (j.eq.1) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i+1,j,k),u(i,j+1,k),u(i-1,j,k),u(i-1,j+1,k),u(i+1,j+1,k))
            end do
          else if (j.eq.nj) then
            do k=1,4
              um(i,j,k) = min(u(i,j,k),u(i+1,j,k),u(i-1,j,k),u(i,j-1,k),u(i+1,j-1,k),u(i-1,j-1,k))
            end do
          else
            do k=1,4
