program shocktube
  ...
  implicit none
  double precision :: a,rho_inf,p_inf,u_inf,v_inf,x0,y0,K,xb,yb,rb,temp,g,theta
  ...
  ! Reading mesh
  call read_from_file(grid,"tube_rotated.cgns")
  ...
  do j=1,grid%nelemj
    do i=1,grid%nelemi
      ...
      if ((grid%elem(i,j)%xc*cos(theta)+grid%elem(i,j)%yc*sin(theta)).le.0.5d0) then
        w0(i,j,1) = 1.0d0
        w0(i,j,2) = 0.0d0
        w0(i,j,3) = 0.0d0
        w0(i,j,4) = 1.0d0
